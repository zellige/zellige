...
makeAPass bb lines = filter removeOutside <$> outCodeForLines bb lines
  where
...
outCodeForLines bb = fmap (fmap (fmap (\(p1, p2) -> (toP1 bb p1, toP2 bb p2))) getLines)
  where
